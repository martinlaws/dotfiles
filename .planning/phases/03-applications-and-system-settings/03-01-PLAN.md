...
### Creative ###
cask "figma@beta"            # (recommended) Design tool (Beta)
...
Note: Claude Code is installed via CLI (brew "claude" in Brewfile), not cask.
  ...
  <verify>
cat config/Brewfile.apps | grep -c "cask" returns count of casks (should be ~20)
...
<success_criteria>
- Brewfile.apps contains all 20 apps from context in 7 categories
