...
must_haves:
  ...
  artifacts:
    - path: "scripts/lib/state.sh"
      provides: "JSON state file read/write/update functions using jq"
      exports: ["state_init", "state_read", "state_write", "state_get_last_run", "state_set_phase_complete"]
...
<objective>
Create the state management and backup infrastructure for update mode detection and safe modifications.

Purpose: Enable the setup script to detect previous runs and track what was installed, while ensuring all modifications have recoverable backups.
Output: Two library scripts (state.sh, backup.sh) that other update scripts will source.
</objective>
...
<task type="auto">
  <name>Task 1: Create state management library</name>
...
Verify JSON is valid and has expected structure.
  ...
  <done>
state.sh exists with all functions, creates valid JSON state file, state_exists() correctly detects first-run vs update mode.
  </done>
</task>
...
<task type="auto">
  <name>Task 3: Add jq to Brewfile if missing</name>
...
- [ ] state_init creates ~/.local/state/dotfiles/setup-state.json with valid JSON
...
2. After setup completes: state file exists with last_run timestamp
