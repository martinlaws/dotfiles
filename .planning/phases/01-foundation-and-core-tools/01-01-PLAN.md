...
must_haves:
  truths:
    ...
    - "Setup is idempotent - re-running skips already-installed Homebrew"
  artifacts:
    - path: "setup"
    ...
    - path: "scripts/install-homebrew.sh"
      provides: "Homebrew installation with UI"
      contains: "NONINTERACTIVE=1"
...
<objective>
Create project structure and implement Homebrew installation with beautiful CLI interface.

Purpose: Establish the foundation that all subsequent phases build upon - directory structure, reusable UI functions, and Homebrew as the package manager.

Output: Working setup entry point that installs Homebrew with progress indicators, architecture detection, and immediate PATH configuration.
</objective>
...
4. **config/Brewfile**:
   - Phase 1 CLI tools per requirements:
     ```
     brew "git"
     brew "node"
     brew "pnpm"
...
IMPORTANT: The setup script must work on first run when gum is NOT installed yet. Use plain echo for pre-gum output, then gum for post-Homebrew output.
  ...
  <done>
    ...
    - scripts/lib/detect.sh detects architecture and sets BREW_PREFIX
    - config/Brewfile declares all Phase 1 CLI tools
    - All scripts pass bash syntax validation
  </done>
</task>

<task type="auto">
  <name>Task 2: Implement Homebrew installation with beautiful UI</name>
  <files>
    scripts/install-homebrew.sh
    setup
  </files>
  <action>
Create the Homebrew installation script with proper UI and error handling:
...
- Don't use set -e; check each critical command's exit status
  ...
  <done>
    - Homebrew installs successfully on fresh Mac
    ...
    - Re-running setup skips Homebrew installation (idempotent)
...
5. **Full run (on fresh Mac or with Homebrew uninstalled):**
   ```bash
   sh setup
   which brew
   which gum
...
<success_criteria>
- [ ] Running `sh setup` on fresh Mac displays welcome message and installs Homebrew
- [ ] Homebrew installs to correct architecture path
- [ ] gum is installed after Homebrew
- [ ] PATH is configured in current session and .zprofile
- [ ] Re-running `sh setup` skips Homebrew (shows "already installed")
